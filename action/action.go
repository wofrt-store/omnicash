...
type Action struct {
	...
	channelClients  map[string]apitxn.ChannelClient
	channelAdminClients 	map[string]apifabclient.Channel
}
...
func (action *Action) Initialize(flags *pflag.FlagSet) error {
	...
	//fmt.Println("SelectionProvider:",cliconfig.Config().SelectionProvider())
	...
	action.channelClients = make(map[string]apitxn.ChannelClient)
	action.channelAdminClients = make(map[string]apifabclient.Channel)

	var allPeers []apifabclient.Peer
	allPeersByOrg := make(map[string][]apifabclient.Peer)
	for _, orgID := range cliconfig.Config().OrgIDs() {
		cliconfig.Config().Logger().Debug("orgId is [%s]\n", orgID)
		...
		peerUrl := cliconfig.Config().PeerURL()

		var peers []apifabclient.Peer
		for _, p := range peersConfig {

			array := strings.Split(peerUrl, ",")
			if len(peerUrl) > 0 {
				includeFlag := false
				for _, v := range array {
					if p.URL == v {
...
func (action *Action) AsBytes(args []string) [][]byte {
...
func (action *Action) ChannelClient() (apitxn.ChannelClient, error) {
	return action.GetChannelClient(cliconfig.Config().ChannelID())
}

func (action *Action) GetChannelClient(channelID string) (apitxn.ChannelClient, error) {
	_client, ok := action.channelClients[channelID]
	if ok && _client != nil{
		//fmt.Println("get cached channelClients()")
		return _client,nil
	} else {
		cliconfig.Config().Logger().Println("Creating new channel client for channel ", channelID)
		...
		client,err := action.sdk.SessionFactory.NewChannelClient(action.sdk, session, action.sdk.ConfigProvider(), channelID)
		if err != nil {
			return nil, errors.Errorf("error new channel client: %s", err)
		}
		action.channelClients[channelID] = client
		return client,nil
	}
}


func (action *Action) GetOrgAdminChannelClient(channelID,orgID string) (apifabclient.Channel, error) {
	_client, ok := action.channelAdminClients[channelID]
	if ok && _client != nil{
		//fmt.Println("get cached AdminChannelClient()")
		return _client,nil
	} else {
		cliconfig.Config().Logger().Println("Creating new admin channel client for channel ", channelID," and org ", orgID)
		//fmt.Println("newAdminChannelClient()")
		...
		action.channelAdminClients[channelID] = channelClient
...
func (action *Action) OrgAdminChannelClient(orgID string) (apifabclient.Channel, error) {
	...
	return action.GetOrgAdminChannelClient(channelID,orgID)
}
...
func (action *Action) OrgUser(orgID, userName string) (apifabclient.User, error) {
	cliconfig.Config().Logger().Debugf("OrgUser for org %s: userName %s", orgID, userName)
	if userName == "" {
		return nil, errors.Errorf("no user name specified")
	}


	//cliconfig.Config().Logger().Debugf("action.sdk.ConfigProvider:", action.sdk.ConfigProvider())
	//cliconfig.Config().Logger().Debugf("action.sdk.CryptoSuiteProvider:", action.sdk.CryptoSuiteProvider())
...
func getPeers(allPeers []apifabclient.Peer, peerURLs string) ([]apifabclient.Peer, error) {
	...
	cliconfig.Config().Logger().Debugf("inputed peer: %s\n", s)
	for _, peer := range allPeers {
		cliconfig.Config().Logger().Debugf("configed peer: %s\n", peer.URL())
