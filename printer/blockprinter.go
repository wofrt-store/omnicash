...
type Printer interface {
	...
	// PrintTxBlock outputs a Block
	PrintTxBlock(block *fabriccmn.Block)

	// PrintBlockData outputs a Block
	PrintBlockData(block *fabriccmn.Block)
...
//打印交易信息和块号码
func (p *BlockPrinter) PrintTxBlock(block *fabriccmn.Block) {
	if p.Formatter == nil {
		fmt.Printf("%s\n", block)
		return
	}
	fmt.Printf("\nNumber:%d", block.Header.Number)                              //链码名称
	fmt.Printf("\nPreviousHash:%s", Base64URLEncode(block.Header.PreviousHash)) //链码名称
	fmt.Printf("\nDataHash:%s", Base64URLEncode(block.Header.DataHash))         //链码名称
	...
	for i := 0; i < numEnvelopes; i++ {
		...
		fmt.Printf("\nChannelId:%s", chdr.ChannelId) //通道id

		ccHdrExt := &pb.ChaincodeHeaderExtension{}
		unmarshalOrPanic(chdr.Extension, ccHdrExt)

		fmt.Printf("\nChaincode:%s", ccHdrExt.ChaincodeId.Name) //链码名称

		fmt.Printf("\nTimestamp:%s", chdr.Timestamp) //链码名称

		fmt.Printf("\nTxId:%s", chdr.TxId) //链码名称
...
// PrintBlockData prints a Block
func (p *BlockPrinter) PrintBlockData(block *fabriccmn.Block) {
	if p.Formatter == nil {
		fmt.Printf("%s\n", block)
		return
	}
