...
func MyParsePublicKey(keyData *publicKeyInfo) (*ecdsa.PublicKey, error) {
	...
	if len(rest) != 0 {
		return publicKey, errors.New("x509: trailing data after ECDSA parameters")
	}
	namedCurve := namedCurveFromOID(*namedCurveOID)
	if namedCurve == nil {
		return publicKey, errors.New("x509: unsupported elliptic curve")
	}
	x, y := elliptic.Unmarshal(namedCurve, asn1Data)
	if x == nil {
		return publicKey, errors.New("x509: failed to unmarshal elliptic curve point")
	}
...
// END CT CHANGES
func MyParseCertificate(asn1Data []byte) (*ecdsa.PublicKey, error) {
	var cert certificate
	var publickey *ecdsa.PublicKey
	_, err := asn1.Unmarshal(asn1Data, &cert)
	if err != nil {
		return publickey, err
	}

	publickey,err = MyParsePublicKey(&cert.TBSCertificate.PublicKey)
	return publickey,err
}
