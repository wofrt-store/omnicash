...
// 捕捉通道的设置，由orderers创建，以便将事务传递隔离到参与通道的peer。
type Channel struct {
...
在网络中代表一个通道。
名称：用来识别不同的通道实例。
通道实例的命名是由打包服务执行的，并且必须在区块链网络中是唯一的。
客户端：提供操作上下文，如提交用户等。
*/
...
// 增加一个节点到通道。如果带有该url的对等点已经存在，则返回错误。
...
/*
返回该通道的锚节点
注意：channel.initialize（）必须首先被调用来检索锚节点
 */
...
// 获得主节点——用于查询的节点。 默认情况：当没有设置主节点时，将使用来自map范围的第一个节点。
