...
type EventHub struct {
	...
	// Array of clients registered for invoke block events
	invokeRegistrants []func(*common.Block)
...
func NewEventHub(client fab.FabricClient) (*EventHub, error) {
	...
	eventHub.RegisterInvokeEvent(eventHub.txCallback)
	return &eventHub, nil
}
...
// RegisterBlockEvent - register callback function for invoke block events
func (eventHub *EventHub) RegisterInvokeEvent(callback func(*common.Block)) {
	eventHub.mtx.Lock()
	defer eventHub.mtx.Unlock()

	eventHub.invokeRegistrants = append(eventHub.invokeRegistrants, callback)

	// Register interest for blocks (only declare interest once, so do this for the first registrant)
	if len(eventHub.invokeRegistrants) == 1 {
		eventHub.interestedEvents = append(eventHub.interestedEvents, &pb.Interest{EventType: pb.EventType_BLOCK})
	}
}

// UnregisterBlockEvent unregister callback for block event
func (eventHub *EventHub) UnregisterInvokeEvent(callback func(*common.Block)) {
	eventHub.mtx.Lock()
	defer eventHub.mtx.Unlock()

	f1 := reflect.ValueOf(callback)

	for i := range eventHub.invokeRegistrants {
		f2 := reflect.ValueOf(eventHub.invokeRegistrants[i])
		if f1.Pointer() == f2.Pointer() {
			eventHub.invokeRegistrants = append(eventHub.invokeRegistrants[:i], eventHub.invokeRegistrants[i+1:]...)
			break
		}
	}
	...
	// Unregister interest for blocks if there are no more registrants
	if len(eventHub.invokeRegistrants) < 1 {
		invokeEventInterest := pb.Interest{EventType: pb.EventType_BLOCK}
		eventHub.grpcClient.UnregisterAsync([]*pb.Interest{&invokeEventInterest})
		for i, v := range eventHub.interestedEvents {
			if *v == invokeEventInterest {
				eventHub.interestedEvents = append(eventHub.interestedEvents[:i], eventHub.interestedEvents[i+1:]...)
			}
		}
	}
